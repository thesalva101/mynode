...
impl RoleNode<Candidate> {
    ...
    pub fn tick(mut self) -> Result<Node, Error> {
        ...
        if self.role.election_ticks >= self.role.election_timeout {
            info!(
                "Election timed out, starting new election for term {}",
                self.term + 1
            );
            self.init()?;
        }
    ...
    fn process_event(mut self, msg: Message) -> Result<Node, Error> {
        match msg.event {
            ...
            Event::AcceptEntries { .. } => {}
            Event::RejectEntries { .. } => {}
            // TODO: FIXME These should be queued or something
            Event::ReadState { .. } => {}
            Event::MutateState { .. } => {}
            Event::RespondState { .. } => {}
            Event::RespondError { .. } => {}
